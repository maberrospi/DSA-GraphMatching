...
def main(match_type="single", load_segs=True):
    ...
    # Get the tiled (patched) versions of our images
    ...
    if match_type == "patched":
        patch_match(segm_pre_post, feat_map_pre_post)
        return
    ...
    if match_type == "rand_patch":
        ...
        # If I want to use all and combine afterwards
        # I need to convert from an index used in the loop
        # to x,y positions to use for offsets.
    ...
    if match_type == "rand_patch":
        ...
        # print(patched_feat_maps[0].shape)
        # print(pre_feat_map.shape)
    ...
    for _, scale in enumerate(scales):
        ...
        post_r_avg, post_r_max, post_r_min, post_r_std = GMatch.calc_graph_radius_info(
            cur_post_graph
        )
        print(
        ...
        # Transpose keypoints if necessary - Maybe not necessary.
        # if transposed:
        #     temp_kpts = kpts1_tmp.copy()
        #     kpts1_tmp = kpts2_tmp.copy()
        #     kpts2_tmp = temp_kpts.copy()
...
def patch_match(
    pre_post_evt: list[np.ndarray], pre_post_feat_maps: list[np.ndarray]
) -> None:
    ...
    for p_idx in range(0, for_range):
        ...
        # Used for looping multiple times if patched or once if otherwise.
        # for_range = 1
        ...
        for _, scale in enumerate(scales):
            ...
            # matchings = GMatch.spectral_matcher(
            ...
            # Plot intermediate patch results
            GMatch.multiscale_draw_matches_interactive(
                cur_pre_graph,
                cur_post_graph,
                pre_post_evt[0],
...
if __name__ == "__main__":
    main(match_type="patched",load_segs=True)
    # patch_match(match_type="patched",load_segs=True)
